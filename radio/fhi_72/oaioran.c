...
void oai_xran_fh_rx_callback(void *pCallbackTag, xran_status_t status, uint8_t mu)
{
  ...
  if (rx_sym != 7) { // if not called for full slot, nothing to process and return
    return;
  }
  ...
  for (uint16_t cc_id = 0; cc_id < 1 /*nSectorNum*/; cc_id++) { // OAI does not support multiple CC yet.
    for (uint8_t ant_id = 0; ant_id < num_rx_ant; ant_id++) {
      uint8_t *rx_data = (uint8_t *)ru->common.rxdataF[ant_id + (num_rx_ant * port_id)];
      ...
      for (int32_t sym_idx = 0; sym_idx < XRAN_NUM_OF_SYMBOL_PER_SLOT; sym_idx++) {
        ...
        p_rx_packet_ctl->nRxPkt = 0; // for the next (slot + XRAN_N_FE_BUF_LEN) slot
        LOG_D(HW, "nRxPkt %d\n", nRxPkt);
 ...
 */
static void fh_on_timing_sync(void *param,
                               uint64_t gps_second,
...
/** @details Fires every TTI. First invocation initializes RU timing from the
 * GPS-anchored xRAN TTI boundary; every subsequent invocation wakes
 * oran_slot_thread (via ru->oran_slot_indicate) to drive the L1 clock. */
int oai_physide_dl_tti_call_back(void *param, uint8_t mu)
{
  static bool timing_init_done = false;
  oran_port_instance_t *pi = (oran_port_instance_t *)param;
  const int slots_in_sf = 1 << mu;

  /* Use xran_get_slot_idx to read the current OTA slot directly from xRAN's
   * timing source.  Reading pi->pusch_tag.slotiId here would give a stale
   * value written by rx_ul_deadline_full_cb (UL timing), which is offset
   * from the true OTA/DL timing by a deadline-advance amount that does not
   * necessarily match sl_ahead — causing the PRACH schedule to be off by
   * a systematic number of slots in steady state. */
  uint32_t frame, subframe, slot_in_sf;
  uint64_t second;
  xran_get_slot_idx(pi->pusch_tag.oXuId, &frame, &subframe, &slot_in_sf, &second, mu);
  uint32_t slot = subframe * slots_in_sf + slot_in_sf;
  uint32_t tti = frame * (uint32_t)(slots_in_sf * 10) + slot;

  if (!timing_init_done) {
    set_oran_ru(RC.ru[0]);
    fh_on_timing_sync(g_ru, second, tti, (int)frame, (int)slot);
    timing_init_done = true;
    LOG_I(HW, "timing sync from physide DL TTI cb: %u.%u\n", frame, slot);
    return 0;
  }

  if (!g_ru || !g_ru->oran_slot_indicate)
    return 0;

  NR_DL_FRAME_PARMS *fp = g_ru->nr_frame_parms;
  openair0_timestamp_t ts = (openair0_timestamp_t)frame * fp->samples_per_subframe * 10
                            + get_samples_slot_timestamp(fp, slot);
  g_ru->oran_slot_indicate(g_ru, (int)frame, (int)slot, ts);
  return 0;
}
