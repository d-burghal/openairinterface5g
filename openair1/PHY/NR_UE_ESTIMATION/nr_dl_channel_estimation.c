                              ...
                              c16_t rxdataF[][ue->frame_parms.samples_per_slot_wCP])
{
  ...
  for (int l = prs_cfg->SymbolStart; l < prs_cfg->SymbolStart + prs_cfg->NumPRSSymbols; l++) {
    ...
    printf("[gNB %d][rsc %d] PRS config l %d k_prime %d:\nprs_cfg->SymbolStart %d\nprs_cfg->NumPRSSymbols %d\nprs_cfg->NumRB %d\nprs_cfg->CombSize %d\n", gNB_id, rsc_id, l, k_prime, prs_cfg->SymbolStart, prs_cfg->NumPRSSymbols, prs_cfg->NumRB, CombSize);
#endif
    ...
    for (int rxAnt = 0; rxAnt < frame_parms->nb_antennas_rx; rxAnt++) {
      ...
      // calculate RE offset
      int k = (prs_cfg->REOffset + k_prime) % CombSize + prs_cfg->RBOffset * 12 + frame_parms->first_carrier_offset;
  ...
  for (int rxAnt = 0; rxAnt < frame_parms->nb_antennas_rx; rxAnt++) {
    ...
    c16_t chF_interpol[frame_parms->nb_antennas_rx][dft_sz] __attribute__((aligned(32)));
    ...
    if(first_half > 0)
      memcpy(&chF_interpol[rxAnt][start_offset], &ch_tmp_buf[0], first_half * sizeof(c16_t));
    if(second_half > 0)
      memcpy(&chF_interpol[rxAnt][0], &ch_tmp_buf[first_half], second_half * sizeof(c16_t));

    c16_t chT_interpol[frame_parms->nb_antennas_rx][dft_sz] __attribute__((aligned(32)));
    // Convert to time domain
    freq2time(NR_PRS_IDFT_OVERSAMP_FACTOR * symb_sz,
    ...
    int prs_toa, ch_pwr;
    peak_estimator(chT_interpol[rxAnt], NR_PRS_IDFT_OVERSAMP_FACTOR * symb_sz, &prs_toa, &ch_pwr, mean_val);

    // adjusting the rx_gains for channel peak power
    double sdr_gain = openair0_cfg[ue->rf_map.card].rx_gain[0] - openair0_cfg[ue->rf_map.card].rx_gain_offset[0];
    double ch_pwr_dbm = 10 * log10(ch_pwr) + 30 - SQ15_SQUARED_NORM_FACTOR_DB - sdr_gain - dB_fixed(symb_sz);

    prs_meas[rxAnt]->rsrp_dBm = 10 * log10(prs_meas[rxAnt]->rsrp) + 30 - SQ15_SQUARED_NORM_FACTOR_DB - sdr_gain
    ...
    LOG_M(filename, varname, &chF_interpol[rxAnt][start_offset], symb_sz, 1, 1);
    sprintf(filename, "%s%i%s", "prsChestT_", rxAnt, ".m");
    sprintf(varname, "%s%i", "prsChT_", rxAnt);
    LOG_M(filename, varname, &chT_interpol[rxAnt][start_offset], symb_sz, 1, 1);
#endif
    ...
    T(T_UE_PHY_INPUT_SIGNAL,
      ...
      T_INT(rxAnt),
      T_BUFFER(&rxdataF[rxAnt][0], frame_parms->samples_per_slot_wCP * sizeof(c16_t)));
    ...
    T(T_UE_PHY_DL_CHANNEL_ESTIMATE_FREQ,
      ...
      T_INT(proc->nr_slot_rx),
      T_INT(rxAnt),
      T_BUFFER(&chF_interpol[rxAnt][0], dft_sz * sizeof(c16_t)));
    ...
    T(T_UE_PHY_DL_CHANNEL_ESTIMATE,
      ...
      T_INT(proc->nr_slot_rx),
      T_INT(rxAnt),
      T_BUFFER(&chT_interpol[rxAnt][0], dft_sz * sizeof(c16_t)));
  }
