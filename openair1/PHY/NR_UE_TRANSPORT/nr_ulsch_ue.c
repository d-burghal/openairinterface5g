                            ...
                            bool was_symbol_used[NR_SYMBOLS_PER_SLOT])
{
  ...
  if (T_ACTIVE(T_UE_PHY_UL_PAYLOAD_TX_BITS)) {
    ...
    T(T_UE_PHY_UL_PAYLOAD_TX_BITS,
      ...
      T_INT((int)get_num_dmrs(pusch_pdu->ul_dmrs_symb_pos)),
      // dmrs_start_ofdm_symbol
      // dmrs_duration_num_ofdm_symbols
      // dmrs_num_add_positions
      T_INT((int)get_dmrs_port(0, pusch_pdu->dmrs_ports)), // dmrs_antenna_port
      T_INT((int)pusch_pdu->scid), // dmrs_nscid
      T_INT((int)frame_parms->nb_antennas_tx), // number of tx antennas
      T_INT((int)pusch_pdu->pusch_data.tb_size << 3), // number_of_bits
      T_BUFFER((uint8_t *)harq_process_ul_ue->payload_AB, pusch_pdu->pusch_data.tb_size << 3 / 8));
  }
#endif
