...
static void init_context_sss_nr(int amp, int N_ID_2, int start_nid1, int nb_nid1, int16_t d_sss[nb_nid1][LENGTH_SSS_NR])
{
  ...
  for (int i = 0; i < nb_nid1; i++) {
    ...
    for (int n = 0; n < LENGTH_SSS_NR; n++) {
      int dss_current = (1 - 2 * x0[(n + m0) % LENGTH_SSS_NR]) * (1 - 2 * x1[(n + m1) % LENGTH_SSS_NR]);
      /* Modulation of SSS is a BPSK TS 36.211 chapter 5.1.2 BPSK */
      d_sss[i][n] = dss_current * amp;
    }
  }
}
                                 ...
                                 c16_t rxdataF[NR_N_SYMBOLS_SSB][params->nb_antennas_rx][params->ofdm_symbol_size])
{
  ...
  init_context_sss_nr((1U << SCALING_PSS_NR) - 1, Nid2, nid1_start, nb_nid1, d_sss);
